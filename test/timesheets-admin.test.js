...
describe("Admin timesheet listing", () => {
  ...
  const createDayWithEntries = async ({ companyId, userId, date, routeOptionId, entries }) => {
  ...
  it("allows admin to list timesheets in their company", async () => {
    ...
    await createDayWithEntries({
      companyId: company.id,
      userId: driver.id,
      date: "2024-05-01",
      routeOptionId: route.id,
      entries: [
        { activityType: "DRIVING", start: 0, end: 60 },
        { activityType: "BREAK", start: 60, end: 90 },
      ],
    });

    const token = (await loginWithSlug({ companySlug: company.slug, identifier: admin.email, password })).body.token;
    const res = await request(app)
      .get("/api/v1/timesheets?from=2024-05-01&to=2024-05-02")
      .set("Authorization", `Bearer ${token}`);

    expect(res.status).toBe(200);
    expect(res.body.timesheets).toHaveLength(1);
    expect(res.body.timesheets[0].route.name).toBe("Route L1");
  ...
  it("scopes to company and does not show other company timesheets", async () => {
    ...
    await createDayWithEntries({
      ...
      userId: driverA.id,
      date: "2024-06-01",
      entries: [{ activityType: "DRIVING", start: 0, end: 30 }],
    });
    await createDayWithEntries({
      companyId: companyB.id,
      userId: driverB.id,
      date: "2024-06-01",
      entries: [{ activityType: "DRIVING", start: 0, end: 45 }],
    });

    const token = (await loginWithSlug({ companySlug: companyA.slug, identifier: adminA.email, password })).body.token;
    const res = await request(app)
      .get("/api/v1/timesheets?from=2024-06-01&to=2024-06-02")
      .set("Authorization", `Bearer ${token}`);
  ...
  it("forbids driver access", async () => {
    ...
    const res = await request(app)
      .get("/api/v1/timesheets?from=2024-07-01&to=2024-07-02")
      .set("Authorization", `Bearer ${token}`);
