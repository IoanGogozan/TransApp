...
const listWorkRunTimesheets = asyncHandler(async (req, res) => {
  ...
  for (const run of runs) {
    ...
    const key = `${date}|${run.userId}|${run.routeOptionId}`;
    if (!rowsByKey.has(key)) {
      rowsByKey.set(key, {
        ...
        route: run.routeOption ? { id: run.routeOption.id, name: run.routeOption.name } : null,
        totalsMinutes: {
          DRIVING: 0,
          OTHER_WORK: 0,
          BREAK: 0,
...
module.exports = { listTimesheets, listWorkRunTimesheets };
